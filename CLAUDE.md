...
## MVP版作成方針
本ファイル（`weather_outfit_advisor.py`）は**プログラミング技術書の初心者向け解説用サンプルコード**として使用するため、以下の方針でMVP（Minimum Viable Product）版を作成する。
...
### MVP化の基本原則
1. **必須機能のみ残す**: 天気取得→アドバイス生成→OLED表示の核心機能
2. **冗長なコメント削減**: 必要最小限の説明コメントに絞る
3. **エラーハンドリング簡素化**: 基本的なtry-exceptのみ
4. **ログ出力削減**: 本番動作に必要なもののみ
5. **設定項目の固定化**: 環境変数の一部をハードコード化検討
6. **docstring簡略化**: 1-2行の簡潔な説明に

### 参照ファイル
- **フル機能版**: `weather_outfit_advisor_full.py`（編集禁止・参照用）
- **MVP版**: `weather_outfit_advisor.py`（編集対象）
...
| ファイル | 説明 |
|---------|------|
| `weather_outfit_advisor_full.py` | OLED版メインプログラム（動作確認済み） |

**注意**: これらのファイルは参照のみ可能です。変更が必要な場合は、コピーを作成して別ファイルで作業してください。

## 参照データディレクトリ
- **パス**: `reference/`
- **用途**: 本プロジェクトで参考にするデータを保存
- **重要**: 参照のみ可。変更・削除・上書き禁止

### reference/06-004-ssd1306-oled-jp-display
- **概要**: SSD1306 OLEDディスプレイに日本語テキストを表示するための参照実装
- **元リポジトリ**: https://github.com/Murasan201/06-004-ssd1306-oled-jp-display
- **本プロジェクトでの利用**: `src/scroll_oled.py`の`OLEDScroller`クラスをライブラリとして使用
- **コピー済みファイル**: `scroll_oled.py`（プロジェクトルートに配置）